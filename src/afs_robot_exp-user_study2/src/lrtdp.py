...
def lrtdp(grid, SEED=42, epsilon=1e-5, max_trials=5, is_oracle=True, use_cache=False, val=0):
    # np.random.seed(SEED)
    ...
    #         target_state = state
    #         break
    # for a in grid.get_actions(target_state):
    #     if is_oracle:
    #         print('state: ', target_state, 'q_val: ', grid.oracle_q_values[(target_state, a)])
    #     else:
    #         print('state: ', target_state, 'q_val: ', grid.agent_q_values[(target_state, a)])
    # print('state: ', target_state, 'action: ', pi[state])
    return pi
