...
class ArmMDP:
    ...
    def get_states(self):
        ...
        States can be represented as follows depending on the domain description:
            - Pushing task:
                (x, y, item_in_contact, surface_type);
                item_in_contact: 0/1; surface_type: 0/1/2;
                mild NSE: (x, y, 1, 1), severe NSE: (x, y, 1, 2), all other cases are no NSE.
        """
        states = []
        for y in range(self.n_rows):
            for x in range(self.n_cols):
                ...
                if currState == 'O':
                    ...
                    obstacle = 0
                    surface = 0
                states.append((y, x, obstacle, surface))
        return states
    ...
    def move(self, currFactoredState, action):
        ...
        if self.is_boundary(new_state):
            ...
            if self.grid_list[new_state[0]][new_state[1]] == 'O':
                return (new_state[0], new_state[1], 1, 0), False
