...
cwd = pathlib.Path(__file__).parent.parent.resolve()  # path to parent of current working directory
...
class FixedBaseRobot:
    ...
    def get_joint_angles(self):
        ...
        """
        joint_states = p.getJointStates(self._id, self._actuated_joints)
...
class Kinova(FixedBaseRobot):
    def __init__(self, base_position=[0.0] * 3):
        f = os.path.join(cwd, "robots", "kinova", "gen3_7dof_robotiq_85.urdf")
        self.urdf_filename = f
        super().__init__(f, base_position=base_position)
